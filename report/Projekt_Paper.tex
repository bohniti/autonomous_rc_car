...
% === ABSTRACT 
\begin{abstract}
For our group IT-project at the \textit{University of Applied Science Georg Simon OHM}, we built an autonomously driving remote control car. To accomplish this project, we used the framework \textit{DonkeyCar}. The neural net for this project is based on supervised learning, but we provide related work and projects to compare this project with the \textit{state-of-the-art} Reinforcement Learning. We explain how to connect the car with the microcontrollers and cables, to be able to rebuild this project. Every single component will be introduced in an appropriate complexity for non-electrical-engineers.  
The neural network is a \textit{VGG16}-Network, which will be explained with all details and parts to fully understand the networks' architecture.
We further provide information how the car can be driving with the webserver. A detailed explanation about the API and the transferred data will be giving in chapter 6.
\end{abstract}
...
The timespan given to us from the university of two semester was sufficient for accomplishing the main objective. The supervised learning on our neuronal networks was successfully tested. Originally we planned to experiment regarding unsupervised learning (Reinforcement Learning), but the given timespan was not sufficient and the complexity was higher than expected. Therefore we conducted a “literature survey” where we evaluated other projects which used reinforcement learning technology and analysed whether the given technology could be implemented in our project or not. 
